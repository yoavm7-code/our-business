...
export class EmailService {
  ...
  constructor() {
    this.fromAddress = process.env.SMTP_FROM || 'FreelancerOS <onboarding@resend.dev>';
  ...
  /** Returns true when a real email provider (Resend or SMTP) is configured. */
  ...
  async sendVerificationEmail(to: string, token: string, nameOrLocale?: string): Promise<void> {
    ...
    const subject = isHe ? 'אימות כתובת אימייל - FreelancerOS' : 'Verify your email - FreelancerOS';
    const html = isHe
      ? `
        <div dir="rtl" style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2>ברוכים הבאים ל-FreelancerOS!</h2>
  ...
  async sendTwoFactorCode(to: string, code: string, method: string): Promise<void> {
    ...
    const subject = isHeLocale ? 'קוד אימות - FreelancerOS' : 'Your verification code - FreelancerOS';
  ...
  async sendLoginAlert(to: string, ip: string, userAgent: string): Promise<void> {
    ...
    const subject = 'Login alert / התראת התחברות - FreelancerOS';
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2>New login to your account</h2>
        <p>A new login was detected on your FreelancerOS account:</p>
        <ul>
        ...
        <div dir="rtl">
          ...
          <p>זוהתה התחברות חדשה לחשבון FreelancerOS שלכם:</p>
          <ul>
    ...
    const html = isHe
      ? `
        <div dir="${dir}" style="font-family: Arial, sans-serif; max-width: 700px; margin: 0 auto; padding: 24px;">
          ...
          <p style="color: #9CA3AF; font-size: 12px; text-align: center;">חשבונית זו הופקה באמצעות FreelancerOS</p>
        </div>
      `
      : `
        <div dir="${dir}" style="font-family: Arial, sans-serif; max-width: 700px; margin: 0 auto; padding: 24px;">
          ...
          <p style="color: #9CA3AF; font-size: 12px; text-align: center;">This invoice was generated by FreelancerOS</p>
        </div>
      `;
