...
export default function IncomePage() {
  ...
  useEffect(() => {
    ...
    categories.list(true).then((c) => setIncomeCategories(c)).catch(() => {});
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="card">
          <p className="text-sm text-slate-500 dark:text-slate-400">{t('income.incomeThisPeriod')}</p>
          {loading ? (
        ...
        <div className="card">
          <p className="text-sm text-slate-500 dark:text-slate-400">{t('income.expensesThisPeriod')}</p>
      ...
      <div className="card max-w-lg">
        <h2 className="font-medium mb-4">{t('income.addIncome')}</h2>
        {!showForm ? (
          ...
          <form onSubmit={handleAddIncome} className="space-y-4">
            ...
            <div className="flex gap-2">
              <button type="submit" className="btn-primary" disabled={saving}>
                {saving ? t('income.saving') : t('common.save')}
              </button>
