...
export default function RecurringPage() {
  ...
  const totalRecurringExpenses = confirmedPatterns
    .filter((p) => p.type === 'expense')
    .reduce((s, p) => s + Math.abs(p.amount), 0);
  const totalRecurringIncome = confirmedPatterns
  ...
  async function handleDismiss(id: string) {
    ...
    try {
      ...
      toast(t('recurring.dismissed'), 'success');
    } catch (err) {
      toast(err instanceof Error ? err.message : t('common.somethingWentWrong'), 'error');
    } finally {
      setActionLoading(null);
  ...
  function formatFrequency(freq: string) {
    ...
    if (freq === 'biweekly') return t('recurring.biweekly');
    if (freq === 'quarterly') return t('recurring.quarterly');
    if (freq === 'yearly' || freq === 'annual') return t('recurring.yearly');
    return freq;
  }
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold">
            {t('recurring.title')} <HelpTooltip text={t('help.recurring')} className="ms-1" />
          </h1>
      ...
      {confirmedPatterns.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          <div className="card">
            <p className="text-sm text-slate-500">{t('recurring.totalPatterns')}</p>
            <p className="text-xl font-bold mt-1">{patterns.length}</p>
          </div>
          <div className="card">
            <p className="text-sm text-slate-500">{t('recurring.recurringIncome')}</p>
            <p className="text-xl font-bold text-green-600 dark:text-green-400 mt-1">
              {formatCurrency(totalRecurringIncome, locale)}
            </p>
          </div>
          <div className="card">
            <p className="text-sm text-slate-500">{t('recurring.recurringExpenses')}</p>
            <p className="text-xl font-bold text-red-600 dark:text-red-400 mt-1">
              {formatCurrency(totalRecurringExpenses, locale)}
            </p>
          </div>
          <div className="card">
            <p className="text-sm text-slate-500">{t('recurring.netRecurring')}</p>
            <p className={`text-xl font-bold mt-1 ${netRecurring >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
              {formatCurrency(netRecurring, locale)}
            </p>
          </div>
        </div>
      )}
      ...
      {currentList.length === 0 ? (
        <div className="card text-center py-12">
          ...
          <p className="text-slate-500">
            {activeTab === 'detected' ? t('recurring.noDetectedPatterns') : t('recurring.noConfirmedPatterns')}
          </p>
          {activeTab === 'detected' && (
            <p className="text-xs text-slate-400 mt-1">{t('recurring.noDetectedHint')}</p>
          )}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {currentList.map((pattern) => {
            const days = daysSince(pattern.lastSeenDate);
              ...
              <div key={pattern.id} className="card animate-scaleIn relative overflow-hidden">
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex items-center gap-2 min-w-0 flex-wrap">
                    ...
                    {/* Stale warning */}
                    {isStale && !pattern.isConfirmed && (
                      <span className="shrink-0 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300">
                        {t('recurring.stale')}
                      </span>
                    )}
                  </div>
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-2 text-sm text-slate-500 mb-4">
                  ...
                  <span className="flex items-center gap-1.5">
                    ...
                    {days > 0 && <span className="text-xs text-slate-400">({days}d)</span>}
                  </span>
                </div>

                {/* Estimated annual impact */}
                <div className="text-xs text-slate-400 mb-3">
                  {t('recurring.estimatedAnnual')}:{' '}
                  <b className={pattern.type === 'income' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}>
                    {formatCurrency(
                      Math.abs(pattern.amount) * (
                        pattern.frequency === 'weekly' ? 52 :
                        pattern.frequency === 'biweekly' ? 26 :
                        pattern.frequency === 'monthly' ? 12 :
                        pattern.frequency === 'quarterly' ? 4 : 1
                      ),
                      locale,
                    )}
                  </b>
